									...
									<?php
										...
										while ($row = mysqli_fetch_array($result)) {
											echo "<tr>
												...
												<td><a class=icon href=records.php?=$row[id]> &#10140; </a></td>
											</tr>";
										}
									?>
...
<?php
	if (isset($_POST['btndelete'])) {
		...
		if (isset($_POST['check'])) {
			...
			for ($i = 0; $i < $cntRec; $i++) {
				$delID = $delRec[$i];
				$sqlDelete1 = "UPDATE tbl_records SET vote = 0 WHERE id = $delID";
				mysqli_query($link, $sqlDelete1);

				$sqlDelete2 = "UPDATE tbl_topics SET respondent = 0 WHERE id = $delID";
				mysqli_query($link, $sqlDelete2);

				$forAlert = true;
			}
			if ($forAlert == true) {
				echo "<script>alert('The survey record/s has been deleted. To delete the topic, go to Survey Topics')</script>";
				echo "<script>window.location.assign('survey_records.php')</script>";
			}
		}
		else {
